...
Lemma Add_IoPE : forall (T : Ensemble (Ensemble Formula)) (Pi A': Ensemble Formula) (f:Formula), 
...
(forall (f : Formula), In Formula Pi f -> exists (A : Ensemble Formula), In (Ensemble Formula) T A /\ In Formula A f) -> Finite Formula Pi -> Finite (Ensemble Formula) (IoPE Pi T).
intros T Pi H H1.
induction H1.
+rewrite IoPE_Empty_set. apply Empty_is_finite.

+rename A into Pi'. rewrite Add_IoPE.
++apply Union_finite_but_not_add.
++apply Pi'.
++apply H0.
...
assert (H1 :exists (n:nat), cardinal Formula Pi n /\ cardinal (Ensemble Formula) (IoPE Pi T) n).
+induction H.
++exists 0. split. -apply card_empty. -unfold IoPE. 
++
